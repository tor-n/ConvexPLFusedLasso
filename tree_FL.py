...
class treeNode():
    def __init__(self, depth=0, sol=[1], lambda_range=[0,float('inf')], lambda_range_type=['in','ex'], 
        ...
        self.depth = depth # the level where this node splits from its parent
...
class Tree():
    ...
    def __init__(self):
        ...
        self.smt = None 
        '''
        # self.ta : array of size N+1 where ta(0) = 0 and ta(i+1) = sum_{j=1}^{i} c_{i,t}
        # self.smt = array of size N+1 where smt(0) =  0 and smt(i+1) = sum_{j=1}^{i} c_{s,i}-c_{i,t} 
        # self.ta and self.smt are updated at every iteration
        self.sol = sol
        self.lambda_range = lambda_range
    ...
    def fit(self, weights, breakpoints):
        self.breakpoints = breakpoints # list of lists
        self.weights = weights # list of lists
        
        N = len(weights)
        Q = sum([len(breakpoints[i]) for i in range(N)])

        self.current_weight_index = [0 for i in range(N)]

        ta = np.zeros(N+1)
        smt = np.zeros(N+1)
        for i in range(1,N+1):
            ta[i] = ta[i-1] + max(0, weights[i-1][0])
            smt[i] = smt[i-1] - weights[i-1][0]
        
        self.ta = ta
        self.smt = smt

        self.order, self.breakpoint_order = self._create_order(breakpoints)

        self.max_depth = Q
        self.num = N
        self.root.sol = [1 for i in range(N)]
        self.root.source_node_interval = [0, N-1]
        self.root.step_move_to_sink = [Q-1 for i in range(N)]
    
    def _create_order(self, breakpoints):
        N = len(breakpoints)
        order = []
        breakpoint_order = []
        h = [(breakpoints[i][0], (i,0)) for i in range(N)]
        heapq.heapify(h)
        while h:
            pop_item = heapq.heappop(h)
            this_var, this_index = pop_item[1][0], pop_item[1][1]
            order.append(this_var)
            breakpoint_order.append(this_index)
            if this_index < len(breakpoints[this_var])-1:
                heapq.heappush(h, (breakpoints[this_var][this_index+1], (this_var, this_index+1)))
        return order, breakpoint_order
    ...
    def _solve_1(self, t, s_int):
        ...
        if r == 1:
            ...
            v_n = ta[r] # nothing moves
            sol_n = [0,-1] # if nothing moves, we always use [0,-1]
            v_e = ta[r] + smt[r] # everything in the s-int moves
            sol_e = [s_int[0], s_int[1]]
            # some but not all nodes move
            argmin_v_r = np.argmin(smt[1:r]) 
            v_r = smt[1+argmin_v_r] + ta[r]
            sol_r = [0,argmin_v_r]
    ...
    def _solve_N(self, t, s_int):
        ...
        if l == N-2:
            v_n = ta[N]-ta[N-1]
            sol_n = [0,-1]
            v_e = v_n+smt[N]-smt[N-1]
            sol_e = [N-1,N-1] 
        else:
            v_n = ta[N]-ta[l+1]
            sol_n = [0,-1]
            v_e = v_n + smt[N]-smt[l+1]
            sol_e = [s_int[0], s_int[1]]
            # some but not all nodes move
            argmin_v_l = l+2+np.argmax(smt[l+2:N])
            v_l = v_n + smt[N]-smt[argmin_v_l]
    ...
    def _solve_middle(self, t, s_int):
        ...
        v_n = ta[r]-ta[l+1]
        sol_n = [0,-1]
        v_e = v_n + smt[r]-smt[l+1]
        sol_e = [s_int[0], s_int[1]]

        if i_t-l > 1:
            argmin_v_l = l+2+np.argmax(smt[l+2:i_t+1])
            v_l = smt[r]+ta[r]-ta[l+1]-smt[argmin_v_l]
            sol_l = [argmin_v_l, s_int[1]]
        if r-i_t > 1:
            argmin_v_r = i_t+np.argmin(smt[i_t+1:r])
            v_r = smt[argmin_v_r+1]-smt[l+1]-ta[l+1]+ta[r]
            sol_r = [s_int[0], argmin_v_r]
        if i_t-l > 1 and r-i_t > 1:
            v_b = smt[argmin_v_r+1]-smt[argmin_v_l]-ta[l+1]+ta[r]
            sol_b = [argmin_v_l, argmin_v_r]
    ...
    def full_split(self):
        ...
        order = self.order
        bp_order = self.breakpoint_order
        N = self.num
        ...
        while node_current_level:
            ...
            # update the smt and ta arrays
            current_weight = weights[order[current_depth]][self.current_weight_index[order[current_depth]]]
            next_weight = weights[order[current_depth]][self.current_weight_index[order[current_depth]]+1]
            
            if current_weight <= 0 and next_weight <= 0:
                # the change affects only source adjacent weight (smt)
                delta = next_weight-current_weight
                self.smt[order[current_depth]+1:] -= delta 
            elif current_weight >= 0 and next_weight >= 0:
                delta = next_weight-current_weight
                self.ta[order[current_depth]+1:] += delta
            else:
                self.smt[order[current_depth]+1:] += (current_weight - next_weight)
                self.ta[order[current_depth]+1:] += next_weight

            
            self.current_weight_index[order[current_depth]] += 1
            ...
            if current_depth == self.max_depth-1:
                ...
                for i, node in enumerate(node_current_level):
                    for j in range(N):
                        all_solutions[i, j] = breakpoints[order[node.step_move_to_sink[j]]][bp_order[node.step_move_to_sink[j]]]
